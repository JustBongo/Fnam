...
import { initAudio, playJumpscareSound, startMenuMusic, stopMenuMusic, playVictoryMusic } from './lib/audio';
...
function WarningScreen() {
   ...
   return (
       ...
       >
          ...
          <p className="max-w-3xl text-2xl leading-relaxed mb-12 font-sans font-black flex flex-wrap justify-center gap-2">
    ...
    <div className="w-full h-screen bg-black flex flex-col items-center justify-center crt relative overflow-hidden">
      ...
      <div className="z-10 flex flex-col items-start w-full max-w-4xl pl-20" onClick={handleInteract}>
        ...
        <div className="flex flex-col gap-6 w-96 text-2xl">
          ...
          <button 
            onClick={() => customNightUnlocked && setShowCustom(true)}
           ...
           <div className="bg-zinc-950 border-2 border-green-500 p-8 flex flex-col gap-4 w-[600px] h-[80vh] overflow-y-auto overflow-x-hidden">
              ...
              <div className="flex flex-col gap-2">
                 ...
                 <div className="flex items-center justify-between bg-zinc-900 p-4">
                    ...
                    <button 
                      onClick={() => buyCameraUpgrade('nightVision')} 
                 ...
                 <div className="flex items-center justify-between bg-zinc-900 p-4">
                    ...
                    <button 
                      onClick={() => buyCameraUpgrade('motionTracked')} 
              ...
              <div className="flex flex-col gap-2 mt-4">
                 <h3 className="text-xl text-white font-mono uppercase border-b border-zinc-700 pb-2">Behind The Scenes</h3>
                 <button 
                    onClick={() => {
                       setShowExtras(false);
                       useGameStore.getState().setGameState('MINIGAME');
                    }}
                    className="flex justify-between items-center bg-zinc-900 hover:bg-zinc-800 p-4 transition-colors text-left"
                 >
                    <div>
                       <div className="text-yellow-500 font-bold uppercase tracking-widest text-xl">"FIND HIM"</div>
                       <div className="text-zinc-500 text-sm">Play the arcade minigame.</div>
                    </div>
              ...
              <button onClick={() => setShowExtras(false)} className="mt-8 border border-green-500 hover:bg-green-900 text-green-400 py-3 font-bold transition-colors tracking-widest text-xl">CLOSE</button>
           </div>
        </div>
      )}
      ...
      {showPatchNotes && (
        <div className="absolute inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm">
           <div className="bg-zinc-950 border-2 border-zinc-700 p-8 flex flex-col gap-4 w-[600px] max-h-[80vh] overflow-y-auto">
              ...
              <button onClick={() => setShowPatchNotes(false)} className="mt-8 border flex items-center justify-center border-zinc-700 hover:bg-zinc-800 text-white py-2 font-bold transition-colors">CLOSE</button>
           </div>
        </div>
      )}
    </div>
...
function GameOver() {
  ...
  return (
    <div className="w-full h-screen bg-black crt flex flex-col items-center justify-center p-8">
      ...
      <div className="z-10 flex flex-col items-center gap-4 mb-12 font-mono uppercase tracking-widest">
        <div className="text-2xl text-[#c084fc] bg-black/50 border border-[#c084fc]/30 px-6 py-4 shadow-[0_0_15px_rgba(192,132,252,0.3)]">
            Time of Death: {displayHour}:{formattedMin}:{formattedSec} AM
        </div>
        {jumpscareBy && (
           <div className="text-3xl text-purple-500 drop-shadow-[0_0_10px_purple] font-black">
               Killed by: {jumpscareBy}
           </div>
        )}
      </div>

      <div className="text-2xl text-zinc-400 z-10 text-center max-w-xl mt-4">
...
function Minigame() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const speed = 2; // px per tick basically
      setPlayerPos(prev => {
        let newX = prev.x;
        let newY = prev.y;
        ...
        // Win condition: reach Golden Marcus
        if (newX > 85 && newY > 40 && newY < 60) {
           setWon(true);
           setTimeout(() => setGameState('MENU'), 3000);
        }
        
        // Hidden Death condition
        if (newX > 75 && newY > 80) {
           setDead(true);
           setTimeout(() => setGameState('MENU'), 3000);
        }
  ...
  }, [won, dead, setGameState]);

  return (
    <div className="w-full h-screen bg-black crt flex items-center justify-center p-8 font-mono overflow-hidden selection:bg-transparent">
       ...
       <div className="absolute top-16 left-10 text-zinc-500 text-lg uppercase tracking-widest">
          Find the yellow suit.
       </div>
       
       <div className="w-[800px] h-[600px] border-4 border-white relative bg-black overflow-hidden scale-110">
          ...
          {/* Player Sprite */}
          <div 
             className={`absolute w-12 h-12 bg-purple-600 transition-all duration-75 ${dead ? 'rotate-90 opacity-50' : ''}`}
             style={{ left: `${playerPos.x}%`, top: `${playerPos.y}%`, transform: dead ? 'translate(-50%, -50%) rotate(90deg)' : 'translate(-50%, -50%)' }}
          >
             {/* Eyes */}
             <div className="w-3 h-3 bg-white absolute top-2 left-2 flex items-center justify-center">
                <div className="w-1 h-1 bg-black" />
             </div>
             <div className="w-3 h-3 bg-white absolute top-2 right-2 flex items-center justify-center">
                <div className="w-1 h-1 bg-black" />
             </div>
             <div className="absolute -bottom-4 left-2 w-2 h-6 bg-purple-600" />
             <div className="absolute -bottom-4 right-2 w-2 h-6 bg-purple-600" />
          </div>

          {/* Goal Sprite (Golden Marcus) */}
          <div className="absolute w-12 h-16 bg-yellow-600 right-[5%] top-[50%] -translate-y-1/2">
          ...
          {/* Hidden danger */}
          <div className="absolute w-16 h-16 bg-red-900 right-[5%] bottom-[5%] rounded-full opacity-20 animate-pulse mix-blend-screen" />
          
          {won && (
             <div className="absolute inset-0 bg-yellow-900/50 flex flex-col items-center justify-center text-4xl text-white font-bold tracking-widest uppercase jrs-glitch backdrop-blur-sm z-50">
                <div className="mb-4">"I WILL PUT YOU BACK TOGETHER"</div>
             </div>
          )}
          {dead && (
             <div className="absolute inset-0 bg-red-900/50 flex items-center justify-center text-6xl text-white font-bold tracking-widest uppercase jrs-glitch backdrop-blur-sm z-50">
                L E A V E
             </div>
          )}
       </div>
